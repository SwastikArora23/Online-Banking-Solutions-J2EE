<%@ page contentType="text/html; charset=iso-8859-1" language="java" import="java.sql.*,java.util.*,onlinebank.BankCommons" errorPage="" %>
<%
//java Code
...
String bal_sql = "SELECT balance FROM tx_details WHERE uname='"+uname+"' AND accno = '"+accno+"' AND isnew = 'YES'";
int bal = BankCommons.getBalance(bal_sql);
String n = BankCommons.getNameByNo("SELECT uname FROM acc_details WHERE acc_no = "+daccno);
String bal_sql_1 = "SELECT balance FROM tx_details WHERE uname='"+n+"' AND accno = '"+daccno+"' AND isnew = 'YES'";
...
if(bal < amt ){
	...
	bal1 = bal - amt;
	String sql_up = "UPDATE tx_details SET isnew = 'NO' WHERE uname='"+uname+"' AND accno = '"+accno+"' AND isnew = 'YES'";
	int up = BankCommons.update(sql_up);
	String sql3 = "INSERT INTO tx_details ( id ,uname, accno, operation, amt, balance, time1, isnew ) VALUES (null,'"+uname+"','"+accno+"','WITHDRAW',"+amt+","+bal1+", '"+date+"', 'YES')";
	int update = BankCommons.update(sql3);
	//deposite into other account
	BankCommons.update("UPDATE tx_details SET isnew = 'NO' WHERE uname='"+n+"' AND accno = '"+daccno+"' AND isnew = 'YES'");
	bal_sql_1 = bal_sql_1 + amt;
	String sql = "INSERT INTO tx_details ( id ,uname, accno, operation, amt, balance, time1, isnew ) VALUES (null,'"+n+"','"+daccno+"','DEPOSIT',"+amt+","+bal_sql_1+", '"+date+"', 'YES' )";
...
<p>&nbsp;</p>	</td>
	...
	<br/>
	<div class="box2">
	Your Amount <font  color="#FF0000"><%=amt%></font> Rs. is Successfully Transfer to Account <font color="#FF0000"><%=daccno%></font><br/>
